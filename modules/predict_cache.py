...
def build_forecast_cache(city: str, zone: str | None, route_id: str, file_name: str):
    ...
    target_dt = pd.Timestamp.now().floor("1H")
    all_new_fc = []

    last_ext_dt = hist["DateTime"].max()
    if pd.isna(last_ext_dt):
        print("❌ Không có dữ liệu DateTime hợp lệ")
        return None
    last_ext_dt = pd.to_datetime(last_ext_dt).floor("1H")

    while last_ext_dt < target_dt:
        remaining_days = int(
            np.ceil((target_dt - last_ext_dt) / pd.Timedelta(days=1))
        )
        if remaining_days <= 0:
            break
        n_days = min(STEP_DAYS, remaining_days)
        ...
        last_ext_dt = pd.to_datetime(hist["DateTime"].max()).floor("1H")
    ...
    *,
    ...
    with lock:
        ...
        if not ext_path.exists():
            ...
            if max_dt is None or pd.to_datetime(max_dt).floor("1H") < target_dt:
                needs_build = True
