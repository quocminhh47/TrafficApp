...
LOOKBACK = 16               # số bước time-series làm input LSTM
...
def build_sequences(F: np.ndarray, y: np.ndarray, lookback: int):
    """
    Tạo X, y_target cho LSTM từ feature matrix F (T, n_feat) và nhãn y (T,):
...
def build_lstm_model(input_shape):
    """
    Xây model LSTM cho binary classification (tắc / không tắc).
    """
    model = Sequential()
...
def train_for_street(df: pd.DataFrame, street_name: str) -> Dict:
    """
    Train & evaluate LSTM cho 1 tuyến.
