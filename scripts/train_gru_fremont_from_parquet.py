# scripts/train_gru_fremont_from_parquet.py
...
def build_gru_model(lookback: int, n_feats: int, horizon: int) -> Model:
    ...
    out = Dense(horizon)(x)
    model = Model(inp, out, name="traffic_gru_fremont_seq2seq")
...
def main():
    ...
    # 3) Chuẩn hoá time-series theo giờ cho mỗi route
    df_all_resampled = []
    for rid in routes:
    ...
    if not all_X:
        raise SystemExit("[ERROR] Không tạo được sample nào cho GRU Fremont.")
    ...
    print("[INFO] Train GRU Fremont xong.")
    ...
    print(f"[INFO] Đã lưu GRU Fremont vào {model_path}")
    print(f"[INFO] Đã lưu scaler Fremont vào {scaler_path}")
    print(f"[INFO] Đã lưu meta Fremont vào {meta_path}")
    print("[DONE] Seattle/FremontBridge family đã sẵn sàng.")
