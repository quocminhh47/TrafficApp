# scripts/train_gru_hcmc_congestion_multi.py
...
CSV_PATH = "../data/raw/hcmc/train.csv"      # chỉnh path nếu cần
LOOKBACK = 16               # số bước time-series làm input GRU
TRAIN_RATIO = 0.8           # 80% train, 20% test
MIN_TIMESTEPS = 80          # tối thiểu số mốc thời gian / tuyến để coi là "đủ dữ liệu"
MIN_POS_RATIO = 0.05        # ít nhất 5% mốc là "tắc"
MIN_NEG_RATIO = 0.05        # ít nhất 5% mốc là "không tắc"

CONGESTED_LOS = {"D", "E", "F"}   # LOS xem là đông/tắc
MODEL_DIR = "../model/hcmc"          # nơi lưu model & metrics
...
def slugify(text: str) -> str:
    """
    Chuyển tên tuyến (có dấu) thành slug safe cho file:
    "Lý Thường Kiệt" -> "ly_thuong_kiet"
    """
    text = unicodedata.normalize("NFKD", text)
    text = text.encode("ascii", "ignore").decode("ascii")
    text = text.lower().replace(" ", "_")
    # bỏ các ký tự đặc biệt hay gặp trong tên đường
    for ch in ["/", "\\", ",", ".", ":", ";", "(", ")", "'", '"']:
...
def build_datetime(df: pd.DataFrame) -> pd.DataFrame:
    """
    Thêm cột DateTime từ cột date + period_x_y
    """
    df = df.copy()
...
def get_eligible_streets(df: pd.DataFrame) -> List[str]:
    """
    Đánh giá tuyến nào "đủ điều kiện":
    - Số mốc thời gian (DateTime) >= MIN_TIMESTEPS
    - Có cả tắc (1) lẫn không tắc (0) đủ tỷ lệ tối thiểu.
    """
    # group theo street_name + DateTime rồi tính LOS
    street_groups = []
    for street, g in df.groupby("street_name"):
        # với mỗi DateTime của tuyến, check tỷ lệ LOS ∈ {D,E,F}
    ...
    # sort theo số mốc thời gian giảm dần cho dễ xem
    ...
    eligible_streets = [s[0] for s in street_groups]
    return eligible_streets


def build_congestion_series(df: pd.DataFrame, street_name: str) -> pd.Series:
    """
    Tạo chuỗi nhị phân tắc/không tắc cho 1 tuyến:
    y_t = 1 nếu >=50% segment tại DateTime đó có LOS ∈ {D,E,F}, ngược lại 0.
...
def build_feature_matrix(series: pd.Series) -> Tuple[np.ndarray, np.ndarray]:
    """
    Từ series nhị phân (index = DateTime), tạo ma trận feature:
    - congested (0/1)
    - sin/cos giờ trong ngày
    - sin/cos thứ trong tuần
    Trả về (F, y) với F.shape = (T, 5).
    """
    s = series.sort_index()
    times = s.index
    ...
    F = np.stack([y, sin_t, cos_t, sin_w, cos_w], axis=1)
    return F, y


def build_sequences(F: np.ndarray, y: np.ndarray, lookback: int):
    """
    Tạo X, y_target cho GRU từ feature matrix F (T, n_feat) và nhãn y (T,):
    X[i] = F[t-lookback : t] ; y_target[i] = y[t]
    """
    T = len(y)
    X_list = []
    y_list = []
    ...
    X = np.stack(X_list)         # (N, lookback, n_feat)
    y_target = np.array(y_list)  # (N,)
    return X, y_target


def build_gru_model(input_shape):
    """
    Xây model GRU cho binary classification (tắc / không tắc).
    """
    model = Sequential()
...
def train_for_street(df: pd.DataFrame, street_name: str) -> Dict:
    """
    Train & evaluate GRU cho 1 tuyến.
    Trả về dict chứa street_name, timesteps, accuracy, f1, path model, v.v.
    """
    print("\n" + "=" * 80)
    print(f"[TRAIN] Tuyến: {street_name}")
    print("=" * 80)
    ...
    print(f"[INFO] Đã lưu model vào: {model_path}")
...
def main():
    ...
    print("\n[INFO] Sẽ train GRU cho các tuyến sau:")
    for st_name in eligible_streets:
        print("  -", st_name)

    # 2) Train lần lượt cho từng tuyến
    all_results = []
    for street_name in eligible_streets:
        res = train_for_street(df, street_name)
        all_results.append(res)

    # 3) Lưu summary metrics
    os.makedirs(MODEL_DIR, exist_ok=True)
    df_metrics = pd.DataFrame(all_results)
    metrics_path = os.path.join(MODEL_DIR, "gru_congestion_metrics.csv")
    df_metrics.to_csv(metrics_path, index=False)
    print(f"\n[INFO] Đã lưu tổng hợp metrics tại: {metrics_path}")
    print(df_metrics)
