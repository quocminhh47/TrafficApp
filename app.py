        ...
        with tab_cmp_daily:
            ...
            if frames:
                ...
                with st.expander(
                    ...
                    # Trung bình theo ngày trong tuần (3 tháng gần nhất)
                    if "Date" in df_eval.columns and not df_eval.empty:
                        df_weekday = df_eval.copy()
                        df_weekday["weekday"] = pd.to_datetime(df_weekday["Date"]).dt.dayofweek
                        metric_cols = [c for c in df_eval.columns if c.startswith("Daily")]

                        if metric_cols:
                            df_weekday_mean = (
                                df_weekday.groupby("weekday")[metric_cols]
                                .mean()
                                .reset_index()
                            )
                            df_weekday_mean["Thứ"] = df_weekday_mean["weekday"].map(weekday_map)
                            order = [weekday_map[i] for i in range(7)]
                            df_weekday_mean = (
                                df_weekday_mean.sort_values("weekday")
                            ...
                            df_weekday_mean = df_weekday_mean.rename(columns=rename_map)

                            chart_col, table_col = st.columns([1, 1])

                            if "Actual" in df_weekday_mean.columns:
                                df_chart_weekday = df_weekday_mean[["Thứ", "Actual"]].copy()
                                chart_weekday = alt.layer(
                                    alt.Chart(df_chart_weekday)
                                    .mark_bar()
                                    .encode(
                                        x=alt.X("Thứ:N", sort=order, title="Ngày trong tuần"),
                                        y=alt.Y(
                                            "Actual:Q",
                                            title="Lưu lượng trung bình (vehicles/ngày)",
                                        ),
                                        tooltip=[
                                            alt.Tooltip("Thứ:N", title="Ngày"),
                                            alt.Tooltip(
                                                "Actual:Q",
                                                title="Lưu lượng trung bình",
                                                format=",.0f",
                                    ...
                                    alt.Chart(df_chart_weekday)
                                    .mark_text(dy=-6, color="#111", fontWeight="bold")
                                    .encode(
                                        x=alt.X("Thứ:N", sort=order),
                                        y="Actual:Q",
                                ...
                                with chart_col:
                                    st.markdown("**Trung bình theo ngày trong tuần (Actual)**")
                                    st.altair_chart(chart_weekday, use_container_width=True)

                            df_table_weekday = df_weekday_mean.copy()
                            for c in df_table_weekday.columns:
                                if c != "Thứ":
                                    df_table_weekday[c] = df_table_weekday[c].round().apply(
                                        lambda v: f"{v:,.0f}" if pd.notna(v) else ""
                                    )

                            weekend_labels = {"Thứ 7", "Chủ nhật"}
                            styler_weekday = df_table_weekday.style.applymap(
                                lambda v: "font-weight: bold" if v in weekend_labels else "",
                                subset=pd.IndexSlice[:, ["Thứ"]],
                            )

                            with table_col:
                                st.markdown("**Bảng trung bình theo ngày trong tuần (3 tháng gần nhất)**")
                                st.dataframe(styler_weekday, use_container_width=True)
            else:
